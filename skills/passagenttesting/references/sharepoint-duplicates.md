...
## Known SharePoint Duplicates in Ensign Therapy Swarm

| SharePoint Folder: Core Clinical Manuals | Duplicate Individual Files to Remove |
|-------------------------------------------|----------------------------------------|
| Medicare Benefits Policy Manual, Chapter 15.pdf | Medicare Benefit Policy Manual Ch.15 (any agent KB) |
| MDS 3.0 RAI Manual 2026.pdf | CMS MDS 3.0 Section GG (SLP/TDA) |
| PDPM-*.pdf (4 files) | Patient-Driven Payment Model + SNF PDPM files |
| Jimmo v. Sebeilis Settlement Agreement.pdf | Jimmo docs + Clinical Decision Support Jimmo-Coverage |
| CFRs that apply to Medicare Part B.docx | 42 CFR 424.24 (check content overlap) |

**Regression Risk:** Removing files that don't exist in SharePoint drops scores. Before removal, verify content existence:
- TDA: Ch.3 Program Integrity NOT in SharePoint → removing regressed conv from 100% → 84%
- TDA: Medicare Secondary Payer Outpatient NOT in SharePoint → removal risk
- Always check SharePoint folder contents before removing individual KB files.
...
5. [ ] Description includes when to use this source
6. [ ] Description includes key topic keywords
7. [ ] Description is unique (not identical to other sources)
8. [ ] File extensions removed from display name