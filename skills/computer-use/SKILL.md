...
Windows terminal chrome/title-bar restore: if the user says the terminal lost its title bar or minimize/maximize/close controls after resize/fullscreen/focus-mode actions, see `references/windows-terminal-titlebar-restore.md` for the Win32 `ShowWindow` + `SetWindowLongW` + `SetWindowPos(SWP_FRAMECHANGED)` recovery pattern. Use this when the desktop driver is unavailable or when restoring the CLI window itself.
