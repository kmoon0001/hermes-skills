...
If the user pastes clinical document text directly into the chat (therapy note content, treatment language, patient functional status, or goal descriptions), treat that text as the submitted document immediately. Proceed with the full Document Review Format: TL;DR → Five‑Section Audit → Compliance Summary Table. Do **not** re‑request a file upload.

**Trigger phrases** for document‑review topics include:
- "here is my note"
- "here is the text"
- "the note reads"
- "I'll paste it here"
- detection of multi‑sentence clinical vocabulary.

**MISSING‑DOCUMENT REVIEW FORMAT** applies only when **no** text content has been provided. If clinical text is already present, skip the invite‑to‑paste step and begin the audit.

Create a `userDocumentText` session variable to capture and persist pasted content across turns, ensuring follow‑up clarifications are appended as context rather than treated as new document requests.

Enforce the **one‑ask rule**: after the agent has requested a document once, if the next message contains clinical text (>30 words, clinical vocabulary), route directly to document analysis instead of prompting again.

---

## CITATION RENDERING ENFORCEMENT

Verify that Generative Answers citation rendering is **ON** in Copilot Studio's Generative AI settings. Every answer **MUST** include at least one inline citation, e.g. `(Medicare Benefit Policy Manual Chapter 15, Section X)` or `(Ensign 7 Habits Framework, Habit X)`. Do not answer without a citation regardless of response length.

Audit all static Topic message nodes (Evaluation, Goals, Progress, etc.) that bypass generative AI and add or update inline citations to comply with G4.

---

## OPERATIONAL QUESTION HANDLING

Document operational knowledge sources (e.g., job ID location, OCR duration) in a quick‑reference guide stored at `~/docs/agent‑ops‑faq.md`.

When guardrails block an operational question, respond with: "I’m unable to answer that directly, but you can find the information here: <link>" and include a link to the appropriate internal documentation.
...
### Update issues
If `hermes update` fails with git permission errors:
1. The remote repository may be using SSH URL without proper key setup
2. Fix: Switch to HTTPS URL: `cd ~/.hermes/hermes-agent && git remote set-url origin https://github.com/NousResearch/hermes-agent.git`
3. Then retry: `hermes update`

If `hermes update` fails with file locking errors (e.g., unable to remove hermes.exe):
1. The Hermes executable may be running in another process
2. Workaround: Start update in a new window and wait:
   ```bash
   cmd /c start hermes update
   timeout /t 10
   hermes version
   ```
3. Or identify and stop the blocking process before retrying
